...
If you want to run a German version of Voicevox, you need to change the docker-compose file to the corresponding [one](docker-compose-de.yml). The TTS is the only thing that's changing, so make sure to also change the `TARGET_LANGUAGE_CODE` in your [.env](./.env) file.
